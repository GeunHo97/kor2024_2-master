...
[ SQL 작성 또는 실행 하는 방법 ]
    ...
    3. 실행할 명령어 줄에 (마우스)커서 두고 ctrl + enter
		- ctrl+enter (번개모양에 I 있는): 명령어 단위 실행 
        - ctrl+shift+enter (번개모양에 I 없는): 현재 sql파일의 모든 sql 실행
    4. 실행 결과는 하단에 [ Result Grid ] 창에서 확인 가능
*/
# [1] db server 내 모든 테이블 확인 # 데이터베이스란? 표/테이블 , 기본 테이블4개의 테이블명이 출력된다.
show databases;
...
# [5] db server 에는 여러개 데이터베이스가 존재하므로 사용/활성화 할 데이터베이스를 선택
use myDBtest1;
# [실습1] - 아래와 같이 순서대로 SQL문을 작성하여 실행하시오.
	-- 1. 'test1' 이름의 데이터베이스가 존재하면 삭제하기
...
/*
    ...
    2. 레코드(Record) : 튜플(tuple) , 행(row) , 가로 1줄
    3. 필드(Field)	: 속성(Attribute) , 열(column) , 세로 1줄 단위
    
    [ 테이블 생성시 필드(데이터) 타입 , = JAVA의 데이터타입과 역할이 같다. oracle vs mysql ]
		[ 정수 ] 
		...
		[실수 = 부동소수점 , 오차 존재 ]
			FLOAT		4바이트
            DOUBLE		8바이트
		[ 문자타입로 저장되는 숫자 , 오차 없는 실수 표현 ]
			DECIMAL
		[ 날짜 ]
			DATE		날짜 표현 , (1000년 ~ 9999년) , 0000-00-00
            DATETIME	날짜와 시간표현 , 0000-00-00 00:00:00
            TIME		시간 표현 , 00:00:00
		[ 문자열 ]
			CHAR( 문자길이 )		: 고정길이 , 최대 1~255글자	, char(5) --> "유재석" --> [유][재][석][ ][ ] 2칸이 남는다.
            VARCHAR( 문자길이 )	: 가변길이 , 최대 1~255글자 , varchar(5) --> "유재석" --> [유][재][석] 남은 칸은 사용하지 않는다.
            TEXT				: 최대길이 6만개 글자표현
            LONGTEXT			: 최대길이 42억개 글자 표현 ( 4GB )
		[ 논리 ]
			BOOL				: true 혹은 false 표현 , 실제로는 0또는 1로 표현된다.
*/
# 테이블 예제
use mydb1120;
# 게시물 테이블
# 1. 게시물 내용물은 많은 양의 데이터를 저장할 예정이라서 longtext 타입 선택 하고 속성명은 content
# 2. 게시물의 작성자는 아이디 이므로 적당한 길이 최대 30글자로 받을 예정이라서 varchar(30) 가변길이의 타입 속성 선택하고 속성명은 writer
# 3. 게시물의 비밀번호는 숫자로 받을 예정 이라서 정수 타입의 int 타입 속성 선택하고 속성명은 password
create table board( content longtext ,  writer varchar(30) , password int );
# 테이블의 내용물 / 레코드 확인
select * from board; # 테이블은 존재하지만  아직 데이터가 없다
drop table if exists board; #테이블 삭제

# 테이블예제2
drop database if exists mydb1120; #1. 데이터베이스 존재하면 삭제
...
create table member( 			  #6. 테이블 생성 : 테이블명( 속성명 속성타입 , 속성명 속성타입 , 속성명 속성타입 )
    ...
    논리필드 bool # 마지막은 , 쉼표 없이 );
);
show tables;					  #7. 테이블목록보기
...
show databases;
show tables;
create database coffeekiosk;
create table coffee(
	name char(30), price int, number int, date char(20)		
);

drop table coffee;
select * from coffee;
insert into coffee (name ,price,number, date )value ("아메리카노" ,1500,2,"2024-11-20"); 
